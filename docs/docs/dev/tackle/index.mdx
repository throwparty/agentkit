...
- `unstable` (default) — enables features not yet covered by the ACP conformance suite, such as session notices and the HTTP transport.
...
The stable-only build (no default features) is also part of the test gate.
