...
```jsonc
{
  ...
  "mcp": {
    // snip
    "litterbox": {
      "command": ["/path/to/agentkit-litterbox", "stdio"],
      "type": "local",
    },
    // snip
  },
...
Create a new agent in your OpenCode configuration directory's `agents` subdirectory. We suggest you name it `litterbox.md`:
...
Restart OpenCode and verify that the agent selector now shows a _Litterbox_ option. You're now ready to [set up your project](../project-setup).
