...
```json
{
  ...
  "context_servers": {
    ...
    "litterbox": {
      "enabled": true,
      "remote": false,
      "command": "/path/to/agentkit-litterbox",
      "args": ["stdio"],
      "env": {}
    }
    // snip
...
Use the `agent: open rules library` command, click _+_ (_New Rule_). Enter the name `Litterbox` and the body:
...
Verify that the agent selector now shows a _Litterbox_ option. You're now ready to [set up your project](../project-setup).
