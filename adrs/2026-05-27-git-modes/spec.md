...
## status: planning created: 2026-05-27 updated: 2026-06-04 author: adrian decision: pending
...
1. **Isolation:** Remote mode sandbox branch operations (create, snapshot, delete) happen in a bare clone. Corruption cannot reach the user's repository.
1. **Seamless history:** Remote mode changes appear in the user's repository. A `litterbox` remote is installed into the user's repo so sandbox branches can be pushed and merged normally.
1. **Zero-risk migration:** Existing direct-mode sandboxes are untouched. Remote mode is opt-in (but will be the default).
1. **Simplicity:** The `Scm` trait is the same in both modes. The mode is an implementation detail of `GitScm::new(mode, config)`.
...
1. Otherwise, default to the **basename** of the VCS root directory (e.g. `/home/user/code/myproject` → `myproject`).
...
1. `GitScm::open(path)` opens the user's repository directly.
1. Creates branch `litterbox/<slug>` directly in the user's repo.
1. Snapshots write to the user's repo.
1. `delete` removes the branch from the user's repo.

### 6.2 `snapshot-mode = "remote"` — First `sandbox-create`

1. User calls `sandbox-create` with name `my-feature`.
1. Litterbox resolves the **project slug** for bare clone path (see §5.1).
1. Checks if a bare clone exists at the project-specific VcsStore path.
1. If not, clones the user's repo as a bare repo there with `--depth 1` (shallow).
1. Installs the bare clone as a `litterbox` remote in the user's repo (if not already present).
1. Creates branch `litterbox/my-feature` inside the bare clone.
1. The GitScm stores `host_repo_path` pointing to the user's working repo so that `make_archive("HEAD")` reads from the correct location (not the bare clone's empty HEAD).
1. Returns `SandboxMetadata` with the project slug.
...
1. Installs the `litterbox` remote (no-op if already present).
1. Creates a new branch inside it.
1. No extra network I/O needed for the clone.
...
1. The snapshot is a full tree of the staging directory — self-contained. Parent reference is for linear ordering, not delta computation.

### 6.5 `delete` — Remote Mode

1. Remove the branch from the bare clone.
1. If this was the last sandbox for the project, remove the bare clone from the VcsStore path.
1. The `litterbox` remote in the user's repo is **never removed automatically** — it persists across all sandbox deletions.
...
1. Branch names use the `litterbox/<slug>` prefix to avoid collisions.
