...
1. `config_loader.load_final()` → `Config { project: { slug: None, snapshot_mode: Remote } }`.
1. `resolve_project_slug()` → `"myproject"` (from current directory basename).
1. `VcsStore::resolve_path("myproject")` → `~/.local/state/agentkit/litterbox/vcs/git/myproject/`.
1. `VcsStore::clone_bare(host_path)` → returns path (clones if missing).
1. `VcsStore::install_remote(host_path, bare_path)` → adds `litterbox` remote.
1. `GitScm::open_with_host(bare_path, ScmMode::Remote, host_path)` → opens bare clone, stores host path for archive reads.
1. `scm.create_branch("my-feature")` → creates `litterbox/my-feature` branch inside the bare clone.
1. `scm.make_archive("HEAD")` → reads from the host repo (via `host_repo_path`), not the bare clone. Required because the bare clone's HEAD is the initial shallow clone, not the user's working tree.
   9a. `MetadataStore::store(project_slug, "my-feature", &metadata)` — persists mode, branch_name, project_slug, container_id.
1. Return `SandboxMetadata { name, branch_name, ..., mode: Remote, project_slug: "myproject" }`.
...
1. Call `VcsStore::clone_bare(host_path, slug)`.
1. Call `VcsStore::install_remote(host_path, bare_path)`.
1. Open `GitScm::open_with_host(bare_path, Remote, host_path)`.
1. For Direct mode, it opens `GitScm::open(".", Direct)` with no host path (unchanged behaviour).

### 4.2 `write` / `commit_snapshot` (remote mode)

1. Mutation happens in staging directory.
1. `GitScm::commit_snapshot_from_staging(staging_path, "message")` opens bare clone.
1. Creates full-tree commit on `litterbox/my-feature` branch.
1. No changes to user's repo.
1. **Note**: `commit_snapshot()` (no staging path) returns `Ok(None)` on bare repos because there is no workdir to snapshot. The snapshot path always goes through `commit_snapshot_from_staging()` in practice.

### 4.3 `delete` (remote mode)

1. `SandboxMetadata.mode == Remote`.
1. `GitScm::delete_branch("my-feature")` → removes branch from bare clone.
1. Check if last sandbox for project (via `list_sandboxes` → filter by `mode == Remote` + project).
1. If last:
...
1. `read`/`write`/`bash`/`patch`: `resolve_sandbox_metadata` calls `MetadataStore::load()`. If the file is absent, falls back to legacy Direct mode.
1. `delete`: `MetadataStore::remove()` is called after the SCM branch is removed. If the file cannot be deleted (permission error), the error is logged but does not block deletion.
...
1. **Phase 2**: Extend `GitScm` with mode field, implement remote mode paths.
1. **Phase 3**: Extend `SandboxMetadata` with new fields, wire configuration.
1. **Phase 4**: Integration tests, self-healing, error recovery.
1. **Final**: Verify all acceptance criteria, direct mode unchanged.
