...
struct AuthConfig {
    ...
    /// OAuth endpoint configuration for providers using OAuth token refresh.
    /// When present, the credential layer will attempt automatic refresh
    /// before forwarding requests. The dispatch is data-driven (presence of
    /// this block), not enum-variant-driven.
...
struct OAuthEndpointConfig {
    ...
    /// OAuth client ID. Optional — falls back to a default per provider.
    client_id: Option<String>,
}

/// AuthType selects an authenticator implementation.
/// Provider-specific auth variants (openai_codex_oauth, anthropic_api_key, oauth_token)
/// have been collapsed into bearer_token — they are all opaque tokens sent as
/// "Authorization: Bearer <value>". OAuth refresh is driven by the presence of an
/// `[auth.oauth]` config block, not by enum variant.
enum AuthType {
    /// "Authorization: Bearer <value>" — all token-based auth
...
sequenceDiagram
    ...
    participant CLI as switchboard auth login
    participant Browser as Browser
    participant OAuth as OAuth Provider
    participant Helper as agentkit-credential-*

    User->>CLI: switchboard auth login openai_codex_sub
    Note over CLI: Read config, find provider<br/>Detect [auth.oauth] block
    CLI->>CLI: Start local HTTP server on :1455
    CLI->>Browser: Open OAuth authorization URL
    Browser->>OAuth: GET authorize_url?client_id=...&redirect_uri=...
    OAuth->>Browser: Login page
    User->>Browser: Log in and grant consent
    Browser->>CLI: Redirect to localhost:1455/auth/callback?code=...
    CLI->>OAuth: POST token_url (exchange code for tokens)
    OAuth-->>CLI: { access_token, refresh_token, expires_at }
    CLI->>Helper: exec store openai_codex_sub (stdin: tokens)
    Helper-->>CLI: OK
...
AGENTKIT_SWITCHBOARD_OPENAI_CODEX_SUB=<access_token>
...
```mermaid
flowchart TD
    Start["Incoming request"] --> Surface["Determine API surface from path"]
    Surface --> Lookup["Look up model in merged model DB"]
    Lookup -- "Not found" --> E404["503 unknown model"]
    Lookup -- "Found" --> Filter["Filter candidates:
    api_surface match
    serves model
    valid credential
    not degraded"]

    Filter --> SessionCheck{"Session ID?"}
    SessionCheck -- "No" --> Rank["Rank providers:
    1° billing model
    2° cost/token
    3° rate-limit headroom
    tiebreaker: identity"]
    SessionCheck -- "Yes" --> Query["Query session DB"]

    Query --> AffinityOK{"Assigned provider<br/>healthy?"}
    AffinityOK -- "Yes" --> Affinity["Use assigned (affinity)
    Skip ranking"]
    AffinityOK -- "No (degraded)" --> Switch["Log switch reason
    Increment switch_count"]
    Switch --> Rank

    Rank --> Assign["Upsert session_affinity row
    (persists across restarts)"]
    Assign --> Select["Select top-ranked provider"]

    Affinity --> Forward["Forward request"]
    Select --> Forward
```
...
Provider-specific quota logic (header parsing, subscription exhaustion, 429 handling) is
encapsulated behind the `ProviderQuotaBehaviour` trait defined in `domain/quota.rs`.
Each provider module under `providers/<name>/quota.rs` implements this trait with its
own header name mapping and subscription model.
...
struct ProviderQuotaState {
    inner: Box<dyn ProviderQuotaBehaviour>,
...
- **Switch breaks are observable:** `switch_count` combined with `routing_events.decision_reason = "fallback"` (session missed — assigned provider degraded) gives enough data to measure cache penalty incidence in production.

#### 8.6.3 Persistence Justification
...
-- Provider degradation state is tracked in-memory by ProviderRegistry
-- with TTL-based recovery; it is not duplicated here.
CREATE TABLE routing_events (
    ...
    decision_reason     TEXT NOT NULL,            -- "affinity", "cost", "fallback"
...
-- Model registry synced from the in-memory ModelDb at startup.
-- Provides a target for session_affinity.model_name FK.
...
#### 8.6.5 Session Lifecycle Management
