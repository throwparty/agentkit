...
async fn test_cassette_replay() {
    let rec = RecordingClient::new(false, "tests/cassettes/echo_tool.json".to_string());
    ...
    let result = agent
        .chat(
            "Call the echo tool with message 'record' and report what it said",
            &mut history,
        )
        .await
    ...
    assert!(
        history.len() >= 2,
        "history should contain at least user+assistant messages"
    );
