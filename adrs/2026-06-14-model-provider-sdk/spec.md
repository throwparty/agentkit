...
## status: draft created: 2026-06-14 updated: 2026-06-14 author: adrian decision: pending
...
1. Compares each candidate against the evaluation criteria (C1–C6)
1. Identifies gaps (features no library provides) that must be built in-house
1. Produces a recommendation with documented rationale
...
1. In each crate, configures the OpenAI provider client
1. Defines a static tool (always returns a hardcoded result)
1. Runs a single prompt turn where the model calls the tool and responds
1. Compares API surface, error handling, compile times, binary size, and documentation quality between the two implementations
...
1. Registers the provider alongside existing built-in providers
1. Runs the same agent loop code without modification
...
1. **A tool call round-trip**: Model requests a tool → the crate executes a statically defined tool (returns a hardcoded result) → model receives the result and produces a final response.
1. **Token usage reporting**: Usage struct inspected and logged after each turn.
...
_(Note: crates live under `adrs/2026-06-14-model-provider-sdk/`, not `samples/`.)_
