...
fn run_store(home: &PathBuf, identity: &str, json: &str) -> bool {
...
fn run_erase(home: &PathBuf, identity: &str) -> bool {
...
fn file_get_store_erase() {
    ...
    assert!(run_erase(&dir, "test_id"), "erase should succeed");
    let (_, success) = run_get(&dir, "test_id");
    assert!(!success, "get after erase should fail");
