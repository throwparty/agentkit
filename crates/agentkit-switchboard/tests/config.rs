...
fn config_parse_bad_enum() {
    let err = agentkit_switchboard::config::loader::load_config(&fixture_path("bad-enum.toml"))
...
fn config_oauth_endpoints() {
    ...
    let oauth = provider
        ...
        .oauth
        .as_ref()
        .expect("should have oauth config");
    assert_eq!(
        oauth.authorize_url,
...
fn config_oauth_default_client_id() {
    let cfg = agentkit_switchboard::config::loader::load_config(&fixture_path(
        "oauth-default-client.toml",
    ))
    .expect("oauth config should parse");
...
fn config_e2e_zen_entries() {
    ...
    let cfg =
        agentkit_switchboard::config::loader::load_config(&path).expect("e2e.toml should parse");
    ...
    let zen_responses = cfg
        .providers
        .get("zen_responses")
        .expect("zen_responses entry");
    ...
    let zen_messages = cfg
        .providers
        .get("zen_messages")
        .expect("zen_messages entry");
