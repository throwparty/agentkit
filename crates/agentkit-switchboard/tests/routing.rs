...
fn make_provider_with_cost(id: &str, billing: BillingModel, cost: f64, models: Vec<&str>) -> (String, ProviderState) {
    (
        id.to_string(),
        ProviderState {
            identity: id.to_string(),
...
fn providers_from(v: Vec<(String, ProviderState)>) -> HashMap<String, ProviderState> {
    v.into_iter().collect()
}
