use crate::auth::OAuthEndpointConfig;
...
fn extract_account_id_from_jwt(token: &str) -> Option<String> {
    // Security note: we intentionally do not verify the JWT signature here.
    // The token is obtained over TLS directly from the trusted token endpoint
    // (authenticated via PKCE code_verifier + client_id). Signature verification
    // via JWKS would be defense-in-depth but is not required for extracting the
    // non-sensitive routing hint `chatgpt_account_id`; a MITM capable of
    // forging tokens already controls the TLS channel and has larger impact.
    // If JWKS verification is desired in the future, gate it behind a feature.
