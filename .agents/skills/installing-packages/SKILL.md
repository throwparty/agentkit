...
## name: installing-packages description: Installing tools and packages via Nix, npm, cargo, pip, and other package managers
...
1. **Cargo** — for Rust crates not in nixpkgs. Use `cargo install` or `cargo binstall` for pre-built binaries.

1. **npm** — for Node.js tools. Use `npm install -g` or `npm install --save-dev` for project-local tools.

1. **pip** — for Python packages (avoid when a nixpkgs equivalent exists). Use `pip install` or `pipx` for isolation.

1. **Direct download** — for statically linked binaries from GitHub releases. Prefer pinning by commit SHA over version tags.
...
1. The `nativeBuildInputs` list in `rustShell`
1. The `commands` string in `rustToolVersions` for version-printing at shell entry
